 ...
 * @return *response.VodGetSpaceConfigResponse, int, error
 */
func (p *Vod) GetSpaceConfig(req *request.VodGetSpaceConfigRequest) (*response.VodGetSpaceConfigResponse, int, error) {
	query := url.Values{}
	form := url.Values{}
	marshaler := protojson.MarshalOptions{
		Multiline:       false,
	...
	respBody, status, err := p.Query("GetSpaceConfig", query)

	output := &response.VodGetSpaceConfigResponse{}
	unmarshaler := protojson.UnmarshalOptions{
		DiscardUnknown: true,
	}
	errUnmarshal := unmarshaler.Unmarshal(respBody, output)
